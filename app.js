...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /**
   * Main Real-Time Simulation Pipeline
   */
  ...
  /**
   * Render Constellations (Tx, Noisy Rx, Equalized Rx)
   */
  function renderConstellations(txSyms, rxFrames, eqSyms, tx) {
    ...
    // 1. Plot Faded Noisy Received Symbols (Red)
    const rxDataSyms = [];
    ...
    // 2. Plot Equalized Symbols (Green)
    ...
    // 3. Plot Transmitted Reference Symbols (Blue Glow)
  ...
  /**
   * Render Time Waveform and Highlight Cyclic Prefix
   */
  function renderWaveform(frameWithCp, cpLen) {
    ...
    // Highlight Cyclic Prefix (CP) Region
    ...
    // Draw Imaginary Waveform (Purple)
  ...
  /**
   * Render Channel Frequency Response & LS Estimate
   */
  function renderChannelResponse(tx, actualTaps, HEstFrame) {
    ...
    // Draw LS Estimated Channel Curve (Gold/Amber)
  ...
  /**
   * Initialize Chart.js BER vs SNR Chart
   */
  function initChart() {
    const ctx = document.getElementById('berChart').getContext('2d');
    
    // Generate static initial data points
    ...
    berChart = new Chart(ctx, {
      ...
      options: {
        ...
        scales: {
          ...
          y: {
            type: 'logarithmic',
            title: { display: true, text: 'Bit Error Rate (BER)', color: '#94a3b8' },
            min: 1e-4,
            max: 1.0,
            grid: { color: 'rgba(255, 255, 255, 0.05)' },
            ticks: { color: '#94a3b8' }
  ...
  /**
   * Run Dynamic BER Sweep over SNR Range
   */
  ...
  /**
   * Initialize Image Transmission Demo (Draw 128x128 test pattern)
   */
  function initImageDemo() {
    ...
    // Draw stylized OFDM text & shapes
  ...
  /**
   * Pass Image Pixels through OFDM Channel & Render Corrupted vs Equalized
   */
  function updateImageTransmissionDemo(tx, taps, snrDb, channelType, estMethod, eqMethod) {
    ...
    // Convert pixel RGB values to bits
    const bits = [];
    for (let i = 0; i < pixels.length; i += 4) {
      // 1 byte per channel (R, G, B)
      const r = pixels[i];
      const g = pixels[i + 1];
      const b = pixels[i + 2];

...
class OFDMTransceiverJS {
  constructor(options = {}) {
    ...
    // Default 4-tap Rayleigh Power Delay Profile
  ...
  mapBitsToSymbols(bits) {
    const nSymbols = Math.floor(bits.length / (this.nData * this.bitsPerSymbol));
    const symbols = [];
  ...
  ifft(freqFrame) {
    ...
    // Conjugate result and divide by N (with sqrt(N) scaling for energy preservation)
    const factor = 1.0 / Math.sqrt(N);
  ...
  // 1D Cooley-Tukey FFT (power of 2)
  fft(x) {
    const N = x.length;
    if (N <= 1) return x;
    if ((N & (N - 1)) !== 0) {
      // Fallback for non-power-of-2: padded simple DFT
      return this.dft(x);
    }
  ...
  passChannel(txSignal, channelTaps, snrDb, channelType = 'RAYLEIGH') {
    ...
    // Calculate signal power & AWGN noise
    let pSignal = 0;
  ...
  estimateChannel(rxFreqFrames, method = 'LS', noisePower = 1e-3) {
    ...
    for (let s = 0; s < nSymbols; s++) {
      ...
      // Linear Interpolation over subcarriers
      const hFrame = new Array(this.nFFT);
      for (let k = 0; k < this.nFFT; k++) {
        // Find adjacent pilots
        let left = hPilots[0];
